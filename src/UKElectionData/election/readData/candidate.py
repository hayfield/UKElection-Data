...
from UKElectionData.election.models import Candidate_2010, Party, Constituency
import csv
...
with open('2010-Data/uk_elections-candidate.csv', 'rb') as f:
    ...
    for row in reader:
        candidate = Candidate_2010(can_id = row[0],
                            can_name = row[1],
                            can_party = Party.objects.get(pk=row[2]),
                            can_votes_2010 = row[3],
                            can_constituency_2010 = Constituency.objects.get(pk=row[4]))
        print candidate
        # candidate.save()
